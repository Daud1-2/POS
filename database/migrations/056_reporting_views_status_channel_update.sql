...
WITH completed AS (
  SELECT
    ...
    SUM(CASE WHEN o.order_channel = 'pos' THEN 1 ELSE 0 END)::bigint AS pos_orders_count,
    SUM(CASE WHEN o.order_channel IN ('online', 'whatsapp', 'delivery_platform') THEN 1 ELSE 0 END)::bigint AS online_orders_count
  FROM orders o
  ...
  SELECT
    o.outlet_id,
    (COALESCE(o.refunded_at, o.updated_at) AT TIME ZONE 'UTC')::date AS business_date,
    COUNT(*)::bigint AS refund_orders_count,
    COALESCE(SUM(o.total), 0)::numeric(14,2) AS refund_amount
  FROM orders o
  WHERE o.deleted_at IS NULL
    AND o.status = 'refunded'
  GROUP BY o.outlet_id, (COALESCE(o.refunded_at, o.updated_at) AT TIME ZONE 'UTC')::date
)
SELECT
  COALESCE(c.outlet_id, r.outlet_id) AS outlet_id,
  COALESCE(c.business_date, r.business_date) AS business_date,
  COALESCE(c.orders_count, 0)::bigint AS orders_count,
  COALESCE(c.subtotal_amount, 0)::numeric(14,2) AS subtotal_amount,
  COALESCE(c.tax_amount, 0)::numeric(14,2) AS tax_amount,
  COALESCE(c.discount_amount, 0)::numeric(14,2) AS discount_amount,
  COALESCE(c.total_collected, 0)::numeric(14,2) AS total_collected,
  COALESCE(c.cash_amount, 0)::numeric(14,2) AS cash_amount,
  COALESCE(c.card_amount, 0)::numeric(14,2) AS card_amount,
  COALESCE(c.online_amount, 0)::numeric(14,2) AS online_amount,
  COALESCE(c.cash_count, 0)::bigint AS cash_count,
  COALESCE(c.card_count, 0)::bigint AS card_count,
  COALESCE(c.online_count, 0)::bigint AS online_count,
  COALESCE(c.pos_orders_count, 0)::bigint AS pos_orders_count,
  COALESCE(c.online_orders_count, 0)::bigint AS online_orders_count,
  COALESCE(r.refund_orders_count, 0)::bigint AS refund_orders_count,
  COALESCE(r.refund_amount, 0)::numeric(14,2) AS refund_amount,
  (COALESCE(c.total_collected, 0) - COALESCE(r.refund_amount, 0))::numeric(14,2) AS net_collected
FROM completed c
FULL OUTER JOIN refunds r
  ON r.outlet_id = c.outlet_id
...
WITH completed AS (
  ...
  SELECT
    o.outlet_id,
    date_trunc('hour', COALESCE(o.refunded_at, o.updated_at) AT TIME ZONE 'UTC') AS bucket_ts,
    COUNT(*)::bigint AS refund_orders_count,
    COALESCE(SUM(o.total), 0)::numeric(14,2) AS refund_amount
  FROM orders o
  WHERE o.deleted_at IS NULL
    AND o.status = 'refunded'
  GROUP BY o.outlet_id, date_trunc('hour', COALESCE(o.refunded_at, o.updated_at) AT TIME ZONE 'UTC')
)
SELECT
  COALESCE(c.outlet_id, r.outlet_id) AS outlet_id,
  COALESCE(c.bucket_ts, r.bucket_ts) AS bucket_ts,
  COALESCE(c.orders_count, 0)::bigint AS orders_count,
  COALESCE(c.revenue, 0)::numeric(14,2) AS revenue,
  COALESCE(r.refund_orders_count, 0)::bigint AS refund_orders_count,
  COALESCE(r.refund_amount, 0)::numeric(14,2) AS refund_amount,
  (COALESCE(c.revenue, 0) - COALESCE(r.refund_amount, 0))::numeric(14,2) AS net_revenue
FROM completed c
FULL OUTER JOIN refunds r
  ON r.outlet_id = c.outlet_id
 AND r.bucket_ts = c.bucket_ts;
...
SELECT
  ...
  COALESCE(SUM(CASE WHEN o.order_channel = 'pos' THEN o.total ELSE 0 END), 0)::numeric(14,2) AS pos_revenue,
  COALESCE(SUM(CASE WHEN o.order_channel IN ('online', 'whatsapp', 'delivery_platform') THEN o.total ELSE 0 END), 0)::numeric(14,2) AS online_revenue
FROM orders o
